...
Assistants require a `First Message` field. This is the first message that is spoken by your assistant when either:

- **A Web Call Connects:** a web call is started with your assistant
- **An Inbound Call Picked-up:** an inbound call is picked-up & answered by your assistant
- **An Outbound Call Dialed & Picked-up:** an outbound call is dialed by your assistant & a person picks up

<Info>
  This first message will not interfere with the normal flow of initiating conversation. Vapi
  dynamically adapts to whoever's turn it is in the dialog to prevent inappropriate interruption.
</Info>
