...
TERMINOLOGY DECIDED (Stephen): simulated agent = "AI tester" (was tester/caller). Tested side = "assistant or squad" (was "target"). Eval-plan rename STILL pending (engineering). Overview/quickstart/advanced already carry these (Stephen's separate-LLM edit pass — DO NOT re-edit those; they're his working-tree versions). Manage page aligned by me (prose "target"->"assistant or squad" + /assistants,/squads links). NOTE: API field is literally `target`/`targetType`/`targetCallId` — keep those in cURL/payloads regardless of display term. Overlay descriptions mostly say "assistant or squad" already; a couple say "target assistant" — align if we want full consistency (not done, out of scope for now).
...
3. PARAMS: DONE/reconciled — mainline now wires `overlays: ./openapi-parameter-descriptions-overlay.yml` in generators.yml. Folded our sim param descriptions INTO that file (14 sim actions: path {id}/{itemId} across personality/scenario/simulation/suite/run + run-item endpoints; query simulationId/runId/status; generate force). Matches mainline format (.* method wildcard, @.name && @.in). Standalone SIMULATIONS-API-PARAM-OVERLAY.yml DELETED. [VERIFY force wording]. Targets no-op until endpoints sync (per Overlay spec zero-match=ignore) — BUILD-TEST to confirm Fern doesn't warn/error on unmatched targets.
...
Overview (Conceptual) + Quickstart (Tutorial) + Advanced (Feature Guide) + Manage (How-to Guide, 4th page — teardown/lifecycle). §3.6 teardown now covered by the Manage page.
- simulations-manage-stub.mdx — How-to Guide (NEW 4th page, Stephen). Edit a suite / Re-run / Cancel a run / Delete a suite, each Dashboard+cURL tabs. cURL verified (PATCH suite optional fields; cancel run = PATCH no body; DELETE suite/{id}; personality/scenario/simulation have own DELETE; runs cancel not delete). CONFIRMED (Stephen screenshots): Re-run = button per run row on Runs page; Delete = trash-can icon in suite header (Composer|Run suite|Edit|trash) -> confirm dialog. Added "Review run results" section (Runs page filters/columns + open run for detail; cURL GET run + GET run/{id} + items). Wired in docs.yml as 4th DRAFT stub. CANCEL confirmed (Stephen): red **Cancel** button in run-detail header while run is queued/running (Re-run disabled until ended). Also: a running run STREAMS live (transcript + evaluations update in real time) — added a "watch live" note to Review run results (relates to the live-listening gap). Manage page now has NO pending dashboard items. ⚠ product bug flag: delete dialog titled "Delete simulation" but deletes the SUITE.
