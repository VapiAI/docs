...
function main() {
  ...
  if (digest(source) !== CLI_SHA256 || source.split(MARKER).length !== 2) throw new Error('Unexpected Fern CLI contents. Refusing to apply the cache adapter.');
  const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'vapi-fern-'));
  const patched = source.replace(MARKER, 'function u8d(args){return globalThis.__vapiFernExamples(args,()=>vapiOriginalExamples(args))}function vapiOriginalExamples({propertySchema:e,breadcrumbs:t,context:r,propertyId:n})');
  try {
