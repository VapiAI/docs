...
Most of the cost of running a voice agent comes from the underlying providers (speech-to-text, LLM, text-to-speech). Vapi adds a per-minute hosting fee on top of those provider costs for the orchestration, quality, and reliability it manages for you.

You pay only for what you use. When you need more support, reliability, or capacity, you can add an optional Success Package.

See the [Vapi pricing page](https://vapi.ai/pricing) for current rates and [Pricing and Success Packages](/billing/pricing-and-success-packages) for how plans work.
...
Yes, Vapi is designed to achieve low latency, typically around 800 milliseconds for end-to-end voice processing. Our infrastructure is optimized for real-time communication, and we continuously work to minimize latency through various optimizations in our pipeline.
  ...
  <Accordion title="Do you offer volume pricing?" icon="chart-line" iconType="solid" defaultOpen={false}>

Yes. Volume pricing on the Vapi hosting fee is available on the Pro and Premier plans through our [sales team](https://vapi.ai/sales). See [Pricing and Success Packages](/billing/pricing-and-success-packages) to compare plans.
