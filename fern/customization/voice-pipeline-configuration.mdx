...
- LiveKit smart endpointing detects when users finish speaking (English only)
...
<Steps>
  ...
  <Step title="Endpointing decision">
    ...
    3. **Smart Endpointing Plan** (`livekit`, `vapi`, or `custom-endpointing-model`)
  </Step>
...
Uses supported smart endpointing providers to predict when users have finished speaking. Use `livekit` for English, `vapi` when you want Vapi-managed smart endpointing in other cases, or `custom-endpointing-model` to call your own endpointing service.
...
<Tabs>
  ...
  <Tab title="Providers">
    **Supported values for `smartEndpointingPlan.provider`:**
    - **livekit**: Recommended built-in smart endpointing provider for English conversations
    - **vapi**: Built-in smart endpointing provider when LiveKit is not the right fit, including non-English conversations
    - **custom-endpointing-model**: Sends endpointing requests to your server and uses the returned timeout

    `krisp`, `deepgram-flux`, and `assembly` are **not** valid values for `smartEndpointingPlan.provider`.
    Configure Deepgram Flux and Assembly on the transcriber instead.
  </Tab>
</Tabs>

**When to use:**

- **Deepgram Flux**: English conversations using Deepgram as a transcriber. Do not set `smartEndpointingPlan`.
- **Assembly**: Best used when Assembly is already your transcriber provider for English conversations with integrated end-of-turn detection. Do not set `smartEndpointingPlan`.
- **LiveKit**: English conversations where you want the recommended built-in smart endpointing provider.
- **Vapi**: English or non-English conversations where you want built-in smart endpointing without running your own service
- **Custom endpointing model**: Conversations that require your own endpointing logic or a custom server integration
...
Use your own endpointing service by setting `provider` to `custom-endpointing-model`. Vapi sends a `call.endpointing.request` event to `assistant.startSpeakingPlan.smartEndpointingPlan.server.url` and expects a timeout in response.
...
```json
{
  "startSpeakingPlan": {
    "smartEndpointingPlan": {
      ...
      "server": {
        "url": "https://your-endpointing-server.example.com"
      }
    }
  }
}
...
- Language-specific endpointing that is not covered by the built-in providers
- Custom ML models or proprietary endpointing services
- Workflows that need endpointing logic outside Vapi's built-in providers
...
**Optimized for:** Teams that want custom endpointing behavior while keeping Vapi's standard interruption controls.
