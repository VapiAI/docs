...
description: Hand off a Vapi call between squad assistants silently, so the caller never hears a transfer, by configuring the destination assistant's first message.
---
