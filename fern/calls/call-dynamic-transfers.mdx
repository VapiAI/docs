...
<Info>
For a lookup that chooses among predefined branch or department phone numbers, follow [Transfer calls using API Request and Transfer Call tools](/calls/transfer-calls-with-api-lookup#choose-a-transfer-approach). Use this guide's backend-controlled pattern when you need asynchronous work or server-controlled transfer timing and destination selection.
</Info>
...
2. **Vapi triggers a Function tool**: Vapi sends the Function tool request to your HTTP server
...
<Tip>
Function tool parameters are fully customizable. Name and structure them to guide routing (for example, `department`, `reason`, `urgency`, or `customerId`).
</Tip>
...
sequenceDiagram
    ...
    Server->>Vapi: POST {controlUrl}<br/>(transfer destination)
...
<Steps>
  <Step title="Create a Function tool for dynamic transfers">
    Create a Function tool that will receive the transfer request and provide you with the control URL to execute the transfer.

    <Tabs>
      <Tab title="Dashboard">
        - Navigate to **Tools** in your dashboard
        - Click **Create Tool** 
        - Select **Function tool** as the tool type
  ...
  <Step title="Create an assistant with the transfer tool">
    <Tabs>
      <Tab title="Dashboard">
        ...
        - Add your `transfer_call` Function tool to the assistant
...
- **Tool call not received**: Verify your server URL is correctly configured in the Function tool and is publicly accessible. Check your server logs for incoming requests.
