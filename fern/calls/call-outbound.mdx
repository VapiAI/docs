...
**▼▼▼ VERSIONING EDIT START — remove before merge ▼▼▼**

## Outbound calls with versioning

By default, an outbound call placed with a saved `assistantId` uses the assistant's **current published version**. To run a specific version instead, specify the version when you create the call.

For the full versioning model — publishing, restoring, and pinning tool versions — see [Versioning](/assistants/versioning). {/* TODO(versioning): add the exact API parameter for selecting a version once the versioning API is finalized */}
