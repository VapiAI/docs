...
By default, an outbound call placed with a saved `assistantId` uses the assistant's **current published version**.{/* Hidden until the versioning API is finalized: "To run a specific version instead, specify the version when you create the call." plus the API parameter for selecting a version. */}
