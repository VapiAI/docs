...
Every call in Vapi ends with an `endedReason` code that tells you exactly why it ended. You can find this value in the **Ended Reason** column of [call logs](/observability/logs/call-logs), or under the `endedReason` field on the [Call object](/api-reference/calls/get).
...
- `call.start.error-enterprise-feature-not-available-recording-consent` — Recording consent requires the Premier plan.
