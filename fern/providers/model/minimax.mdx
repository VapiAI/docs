---
title: MiniMax
subtitle: Use MiniMax for your Vapi voice agent's language model and text-to-speech voice.
description: Connect MiniMax to Vapi to use its language model and text-to-speech voices. Covers supported models, voices, and MiniMax's subtitle timing for live captions.
slug: providers/model/minimax
---

MiniMax is a provider you can use for two parts of a Vapi voice agent: the language model and the voice (text-to-speech). This page covers what MiniMax is and the options each capability exposes.

## What is MiniMax

MiniMax offers a large language model and streaming text-to-speech with multi-language support and voice cloning. On Vapi, you can use MiniMax for the model, the voice, or both on a single assistant.

You can use MiniMax through Vapi's default integration, or use your own MiniMax account by connecting it in [Integrations](/providers/integrations).
...
MiniMax offers many voices and supports voice cloning, so there are too many to list here. Browse and preview them in the **Voice Library** in the [Vapi Dashboard](https://dashboard.vapi.ai), then copy a voice's ID. The default voice is `socialmedia_female_1_v1`.
...
<CardGroup cols={2}>
  <Card title="MiniMax documentation" href="https://www.minimax.io/platform/document/T2A%20V2?key=66719005a427f0c8a5701643">
    The provider's own text-to-speech reference.
  </Card>
