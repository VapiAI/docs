...
<Note>
  In **Dashboard > Integrations > Cartesia**, Vapi currently supports only the
  **API key** field. Vapi does not currently expose a **URL** or custom
  self-hosted Cartesia endpoint field in the dashboard or Cartesia credential
  API, so there is no place to enter a self-hosted Cartesia base URL today.
</Note>
