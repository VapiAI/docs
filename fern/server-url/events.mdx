...
`totalWords: 0` is a valid sentinel on the very first event of a turn before Minimax confirms its word count — guard against divide-by-zero when computing a progress fraction. See the [MiniMax voice provider page](/providers/model/minimax) for full configuration details.
