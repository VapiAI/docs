...
description: Reference the events Vapi POSTs to your Server URL, with the shared message body shape and the payload fields that vary by server message type.
---
...
To inspect recorded server-message deliveries and responses, see [Webhook logs](/observability/logs/webhook-logs).

<Note>
  Monitoring alerts are a separate webhook. When a monitor detects an issue, Vapi sends an `issue.created` message to your notifier endpoint rather than the assistant Server URL. Its payload is documented in [Monitoring](/observability/monitoring-quickstart#webhook-payload).
</Note>
