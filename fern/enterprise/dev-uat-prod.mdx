...
- **▼ VERSIONING EDIT START ▼** **How does this relate to built-in versioning?** Vapi has built-in [versioning](/assistants/versioning) for assistants and the tools they use: publishing creates a new version, and you can restore a previous one. Git-based promotion (this guide) and built-in versioning are complementary — use Git as the cross-environment source of truth, and built-in versions for per-environment history and rollback. **▲ VERSIONING EDIT END ▲** {/* TODO(versioning): tighten once versioning ships and the API is documented */}
