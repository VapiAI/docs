...
description: Create Vapi monitors that evaluate call data against thresholds, open issues when quality drops, and alert your team by email, Slack, or webhook.
---
...
<Warning>
  Monitoring currently requires retained call data. Organizations using Zero
  Data Retention (ZDR) do not receive monitor evaluations because the required
  call logs are not retained.
</Warning>
...
| Field | Meaning |
| --- | --- |
| `message.type` | Always `issue.created`. This is the only monitoring event Vapi emits (see [below](#one-webhook-per-issue)). |
| `message.timestamp` | When the alert was sent, in milliseconds since the Unix epoch. |
| `message.orgId` | The organization that owns the monitor and issue. |
| `message.issue.totalCalls` | Calls evaluated in the window. |
| `message.issue.failingCalls` | Calls that matched the Insight during the evaluation that created the issue. Your failure rate for that evaluation is `failingCalls / totalCalls`. The `callsCount` returned by `GET /monitoring/issue/:id` can increase as later evaluations add affected calls to the active issue. |
| `message.issue.evaluationStartAt` | Start of the evaluation window. |
| `message.monitor.category` | `technical`, `infrastructure`, `effectiveness`, or `compliance`. The payload shape is identical across categories, so this field is how you tell them apart. |
| `message.monitor.trigger.insight.name` | Human-readable name of the metric that breached, for display. |
| `message.monitor.trigger.insight.id` | Stable ID of the configured Insight. Match on this rather than `name` for routing logic. |
| `message.monitor.trigger.threshold` | The comparator and value that were exceeded. |

<Note>
  The webhook reports **only** the breach counts (`totalCalls` and `failingCalls`). It does not include per-call latency (such as p50 or p95), remaining concurrency, or the list of affected calls. To get the individual call IDs behind an issue, call `GET /monitoring/issue/:id` and read the `calls` array (see [Step 3](#step-3-view-issues)).
</Note>
...
| A repeated breach does not create a new issue | A trigger updates the same non-resolved issue across evaluation windows. After you resolve that issue, a later threshold breach can create a new issue. |
