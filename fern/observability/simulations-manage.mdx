...
Open a run to see whether each simulation passed or failed, its evaluations, and its transcript. Open an active run to watch the transcript and evaluations update. If a run or iteration can't complete — for example, no payment method on file — it shows a **Failure reason** describing what went wrong. See the [**Simulations quickstart**](/observability/simulations-quickstart) for the full walkthrough.
...
| Send simulation start and end webhooks | Voice (`vapi.websocket`) | Simulation webhooks trigger only in voice mode. |
