...
During a simulation, the [**assistant**](/assistants) or [**squad**](/squads) under test runs its real [**tools**](/tools/introduction). Mock a tool to return fixed text instead, so that tool never calls its live service or API. A mock intercepts only the tool it names; every other tool the assistant or squad calls still runs for real.
...
- **Tool**: Select one of the [**tools**](/tools/introduction) configured on the assistant or squad. Only its configured tools, including [**custom tools**](/tools/custom-tools), can be mocked.
...
{/* TODO(image): Capture the suite editor "Tool mocks & webhooks" tab showing the Tool mocks section with an example mock configured (Tool, Mock result, Enabled). Save to ./static/images/simulations/tool-mocks.png and replace this comment with a <Frame> per repo convention. */}
...
The mock result is passed to the assistant or squad as the tool's result exactly as entered, so both plain text and JSON strings work. Use tool mocks to keep tool behavior deterministic, avoid side effects, and steer the conversation down a specific path without depending on a live service.
...
{/* TODO(image): Capture the suite editor "Tool mocks & webhooks" tab showing the Webhooks section with "Simulation start" and "Simulation end" toggles. Save to ./static/images/simulations/webhooks-toggles.png and replace this comment with a <Frame> per repo convention. */}
...
The `calls.listenUrl` field contains a WebSocket URL for monitoring the simulation in real time. The `iterationNumber` field identifies the iteration. The ended payload includes `endedReason`, `canceled`, `failureReason`, `transcript`, `messages`, and `recordingUrl`. For the complete list of `endedReason` values, see [**Call ended reasons**](/calls/call-ended-reason).
