...
Versioning lets you change an [assistant](/assistants) or [tool](/tools) without affecting live calls. Publish the draft when it is ready, review earlier versions, or restore an earlier configuration to the draft.

## The versioning lifecycle

Assistants and tools use the same lifecycle:

- **Draft**: Contains your unpublished changes. Draft changes do not affect live calls.
...
- **Restore**: Loads an earlier version into the draft and replaces its current changes. Publish the restored draft to create the next version.

<Note>
The **current** version is the most recently published version. Publishing a draft changes the current version; restoring a version does not change it until you publish the restored draft.
</Note>
