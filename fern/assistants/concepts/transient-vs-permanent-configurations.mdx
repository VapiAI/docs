...
Add call-specific tools directly to a transient assistant's model configuration.

The API Request tool sends the model-generated `body` directly to the configured URL. Replace `YOUR_CREDENTIAL_ID` with a Vapi [Custom Credential](/tools/api-request/configuration#authenticate-requests) that authenticates the request without placing a secret in the tool definition.

The following examples are `assistant.model` fragments:

<CodeBlocks>
```json title="Customer-specific API Request tool"
{
  "tools": [
    {
      "type": "apiRequest",
      ...
      "body": {
        ...
        "properties": {
          ...
          "region": {
            "type": "string",
            "description": "The customer's region code"
          }
        },
...
<Steps>
  <Step title="Create the tool">
    Store the reusable tool on Vapi servers and copy its ID.
  </Step>
  <Step title="Create the assistant">
    Add the tool ID to the assistant's model configuration and copy the assistant ID.
  </Step>
  <Step title="Create calls with the assistant">
    Reference the stored assistant by its ID.
  </Step>
</Steps>
  ...
  -d '{
    ...
    "body": {
      ...
      "properties": {
        ...
        "updates": {
          "type": "object",
          "description": "The contact fields and values to update"
        }
      },
...
Replace the tool, credential, and assistant ID placeholders with values from the resources you created. The credential authenticates the API Request tool's call to the CRM endpoint.
