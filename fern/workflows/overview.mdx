...
Developers can specify a phone number destination and a [transfer plan](/tools/transfer-call/warm-transfer), which lets them specify a message or a summary of the call to the person or agent picking up in the destination number before actually connecting the caller.
