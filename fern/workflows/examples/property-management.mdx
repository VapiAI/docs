...
<Note>
Replace `$VAPI_API_KEY` with your [Vapi API key](/security-and-privacy/api-keys). Update the phone number in the `transferCall` destination to your actual office number.
</Note>
...
* **[Workflow overview](/workflows/overview)** - Review workflow structure and routing concepts
* **[Workflow quickstart](/workflows/quickstart)** - Build and test a workflow from scratch
* **[Migrate to Squads](/workflows/legacy-migration)** - Plan a migration from legacy Workflows to Squads
