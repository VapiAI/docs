...
<Tabs>
  ...
  <Tab title="Web SDK + TypeScript">
    For web-based sales applications:
    ...
    function createSalesWorkflowManager(config: SalesConfig) {
      ...
      vapi.on('call-start', () => {
        isConnected = true;
        console.log('Sales workflow call started');
      });

      vapi.on('call-end', () => {
        isConnected = false;
        console.log('Sales workflow call ended');
      });
      ...
      vapi.on('message', (message) => {
        if (message.type === 'transcript') {
          ...
          handleSalesFunction(message.functionCall);
        } else if (message.type === 'workflow-step') {
          console.log('Sales workflow step:', message.step);
        }
      });
      ...
      function handleSalesFunction(functionCall: { name: string; parameters: Record<string, unknown> }) {
        switch (functionCall.name) {
          case 'lookup_lead':
            console.log('Looking up lead:', functionCall.parameters);
            break;
          case 'qualify_prospect':
            console.log('Qualifying prospect:', functionCall.parameters);
            break;
          case 'schedule_demo':
            console.log('Scheduling demo:', functionCall.parameters);
            break;
          default:
            console.log('Sales function called:', functionCall.name, functionCall.parameters);
        }
      }
    ...
    // Usage for sales workflow
    const salesManager = createSalesWorkflowManager({
      publicApiKey: 'YOUR_PUBLIC_API_KEY',
      workflowId: 'YOUR_SALES_WORKFLOW_ID'
    });

    // Start sales conversation
    salesManager.startSalesCall();
    ```

    <Note>
      For actual file uploads (leads.csv, products.csv, etc.), use the Server SDK or Dashboard. Web SDK is for managing live sales conversations.
    </Note>
  </Tab>
...
<Tabs>
  <Tab title="Web SDK + TypeScript">
    Use the Web SDK to manage sales workflow interactions:
    ...
    function createOutboundSalesWorkflow(config: SalesWorkflowConfig) {
      const vapi = new Vapi(config.publicApiKey);
      let isConnected = false;
      ...
      vapi.on('call-start', () => {
        isConnected = true;
        console.log('Outbound sales call started');
      });

      vapi.on('call-end', () => {
        isConnected = false;
        console.log('Sales call ended');
        processSalesOutcome();
      });
      ...
      function handleSalesFunction(functionCall: { name: string; parameters: Record<string, unknown> }) {
        switch (functionCall.name) {
          case 'qualify_lead':
            console.log('Qualifying lead:', functionCall.parameters);
            break;
          case 'present_product':
            console.log('Presenting product:', functionCall.parameters);
            break;
          case 'schedule_demo':
            console.log('Scheduling demo:', functionCall.parameters);
            break;
          case 'update_crm':
            console.log('Updating CRM:', functionCall.parameters);
            break;
          default:
            console.log('Sales function called:', functionCall.name, functionCall.parameters);
        }
      ...
      function processSalesOutcome() {
        console.log('Processing sales outcome for lead:', currentLead);
      }

      return {
        startSalesCall: (leadData?: any) => {
          if (!isConnected) {
    ...
    // Usage for outbound sales
    const outboundSales = createOutboundSalesWorkflow({
      publicApiKey: 'YOUR_PUBLIC_API_KEY',
      workflowId: 'YOUR_SALES_WORKFLOW_ID'
    });

    // Start sales call
    outboundSales.startSalesCall({
      name: 'Jane Smith',
      company: 'ACME Corp',
    ...
    <Note>
      For web-based sales interfaces, workflows provide structured lead management. Use Server SDK for automated phone-based sales calls.
    </Note>
  </Tab>
  <Tab title="Server SDK + Python">
    ```python
