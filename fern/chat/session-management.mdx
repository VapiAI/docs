...
* **[Session logs](/observability/logs/session-logs)** - Review session status and persisted messages, then use [Chat logs](/observability/logs/chat-logs) to find individual chats by session ID
