...
* A **saved payment method** for a usage-based organization

<Warning>
SMS replies use the Chat API and follow its billing requirements. Usage-based organizations need a saved payment method and, if they are less than 30 days old, one completed payment. Without these, Vapi receives the inbound SMS message and creates a session but does not send a reply or show an error in the Dashboard. Voice calls are not affected. See [Chat troubleshooting](/chat/quickstart#troubleshooting) and [Manage billing and credits](/billing/manage-billing-and-credits).
</Warning>
...
<Tip>
View retained SMS conversations in [**Logs → Sessions**](/observability/logs/session-logs). Each SMS conversation creates a session. While it is retained, you can review its full message history and conversation flow.
</Tip>
...
4. **Session Expiry**: Sessions expire 24 hours after creation by default, then Vapi creates a new session for later conversations
...
* **Returning customers**: Continue the existing session until it expires
* **Session expiry**: After the session expires, Vapi creates a new session automatically
