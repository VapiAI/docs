...
In [**Logs → Calls**](/observability/logs/call-logs), inspect the transfer history and destination content. Check the **Logs** tab for validation, summary-generation, and transfer errors.
...
If the transfer is initiated but the destination never rings or the call drops, follow [Troubleshoot call forwarding drops](/phone-calling/in-call-control/transfer-calls/debug-forwarding-drops).
