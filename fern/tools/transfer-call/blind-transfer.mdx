...
| --- | --- | --- |
| The original call ends, but the destination never rings | The SIP system rejected or did not receive the REFER | Check the affected call's **Logs** tab in [**Logs → Calls**](/observability/logs/call-logs), then inspect the provider or PBX SIP logs and packet capture. Confirm required SIP signaling addresses are allowed. |
...
For additional provider diagnostics and packet capture guidance, follow [Troubleshoot call forwarding drops](/phone-calling/in-call-control/transfer-calls/debug-forwarding-drops).
