...
To choose among stored destinations based on an API lookup, follow [Transfer calls using API Request and Transfer Call tools](/calls/transfer-calls-with-api-lookup). To select or construct a destination at runtime instead of storing it on the tool, follow [Dynamic call transfers](/calls/call-dynamic-transfers).
...
| The call has `assistant-forwarded-call`, but the destination never rings | Vapi initiated the transfer, but the downstream route did not complete it | Check the affected call's **Logs** tab in [**Logs → Calls**](/observability/logs/call-logs) for transfer errors, then check the provider call detail record and transfer compatibility. For SIP calls, inspect the provider's REFER or dial behavior. |
...
For dropped calls, provider diagnostics, and SIP packet checks, follow [Troubleshoot call forwarding drops](/phone-calling/in-call-control/transfer-calls/debug-forwarding-drops). The [call ended reasons reference](/calls/call-ended-reason) lists transfer-specific outcomes such as `call.in-progress.error-transfer-failed`.
...
<CardGroup cols={2}>
  ...
  <Card title="Lookup-based transfers" icon="fa-light fa-magnifying-glass" href="/calls/transfer-calls-with-api-lookup">
    Look up a branch, then transfer to a stored phone number.
  </Card>
  ...
  <Card title="Troubleshoot forwarding drops" icon="fa-light fa-phone-slash" href="/phone-calling/in-call-control/transfer-calls/debug-forwarding-drops">
