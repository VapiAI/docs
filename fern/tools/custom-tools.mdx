...
subtitle: Connect an assistant to your own server logic with a Function tool
slug: tools/custom-tools
description: Create custom tools, including Function tools, for Vapi assistants using the dashboard's visual Tools section or the API to connect to your server-side logic.
---

This guide shows you how to create Function tools for your Vapi assistants. The dashboard picker labels this tool type **Custom Tool**, but the created tool and the API both identify it as `function`. We recommend using the Vapi dashboard's dedicated Tools section, which provides a visual interface for creating and managing tools that can be reused across multiple assistants. For advanced users, API configuration is also available.

<Note>
  Not sure which tool type to use? See [When to use API Request or Function tools](/tools/api-request-vs-function) before configuring your integration.
</Note>
...
1. **Tool Type**: Select **Custom Tool** for custom API integrations
...
### Returning an error instead of a result

If your logic fails, return an `error` field in place of `result` for that tool call — and still respond with **HTTP 200**. Any non-200 status code is ignored entirely, so the assistant hears nothing back and reports "no result returned."
...
Both `result` and `error` must be **single-line strings**. Line breaks (`\n`) cause parsing errors and the response is dropped, so join multi-line output into one line (for example, `"Temperature: 72°F, Condition: Sunny, Humidity: 45%"`).

### Responding to more than one tool call

A single request can contain several tool calls in `toolCallList`. Return one entry in `results` for **every** call, in the same order they were sent, and set each `toolCallId` to exactly match the ID from the request. Individual calls can succeed or fail independently:
...
Return HTTP 200 for the whole response even when an individual tool errors — handle per-call failures with the `error` field, not the status code.

<Note>
  Response coming back empty, ignored, or truncated? See [Troubleshoot tools](/tools/custom-tools-troubleshooting) for the full checklist: response-format rules, tool-call-ID matching, the HTTP 200 requirement, and token limits.
</Note>

### Response format depends on the tool type

The `results` array shown above is the response contract for **Function tools**. Other server-backed tool types can expect a different response shape — a [Query tool](/knowledge-base/using-query-tool) backed by a knowledge base does not necessarily use the `toolCallId`/`result` structure. Follow the guide for the specific tool type you are building rather than assuming the Function-tool format applies everywhere.

{/* TODO(maintainers): a short per-tool-type response-shape table (Function vs Query/Knowledge Base vs Custom TTS/Transcriber) would fully close this gap — "which JSON do I return?" is the single largest Custom Tools confusion in Ask AI. Confirm exact shapes before adding. */}
