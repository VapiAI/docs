...
- **Async Mode**: Enable only for long-running operations where the assistant doesn't need to read the result aloud. With `async: true`, Vapi marks the tool call resolved immediately and the assistant continues the conversation without waiting for your server's response — there's no built-in way to have it speak the result later. Most tools should leave this off (`async: false`, the default) so the assistant waits for your response and speaks the matching `request-complete` or `request-failed` message from [Step 3](#step-3-configure-messages). See [Custom tools troubleshooting](/tools/custom-tools-troubleshooting#async-vs-sync-behavior) for the full behavior comparison.
...
If your tool call fails, return the same `results` array shape but replace `result` with `error`:
...
Vapi decides between the `request-complete` and `request-failed` messages configured in [Step 3](#step-3-configure-messages) by checking for the presence of this `error` field on each result — not by inspecting the content of `result`. Always respond with HTTP 200, even when returning an `error`; any other status code is ignored.
...
<Note>
  Seeing "no result returned", ignored responses, or unexpected async behavior? [Custom tools troubleshooting](/tools/custom-tools-troubleshooting) covers exact response formats, common format mistakes, and async vs. sync behavior in more depth.
</Note>
