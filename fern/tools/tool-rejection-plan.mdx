...
description: Prevent unintended Vapi tool calls with a rejection plan that evaluates recent conversation state and blocks a tool when your conditions are met.
---

## Overview

A rejection plan lets you prevent a tool from executing when certain conditions are met. It is configuration attached to a tool or tool action, not a separate tool in the Dashboard. It evaluates the recent conversation state to decide whether to reject the call.
...
The [transfer call tool](/tools/transfer-call) can use a rejection plan to prevent a transfer when the recent conversation does not meet your conditions.
