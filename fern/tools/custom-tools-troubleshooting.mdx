...
`maxTokens` is a model-level setting on your assistant, not a property of the tool. Raise it on `model` so the model has room to emit full tool parameters and responses:

```json title="Assistant configuration" {5}
{
  "model": {
    "provider": "openai",
    "model": "gpt-4o",
        ...
        "function": {
          ...
          "parameters": {
            ...
            "properties": {
              "query": {
                "type": "string",
                "description": "Detailed request for the tool"
              }
            },
            "required": ["query"]
          }
