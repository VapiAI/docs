...
description: Add Vapi's built-in default tools to assistants, including transfer call, end call, SMS, DTMF keypad, and API request, through the API or dashboard.
---
...
<Info>
To add built-in tools to your agent, you need to add them in the `tools` array of your assistant. You can do this in your api request, or by creating a new tool in the dashboard tools page, and assigning it to your assistant.
</Info>

#### Transfer Call

<Card title="Transfer Call tool" icon="fa-light fa-phone-arrow-right" href="/tools/transfer-call">
  Create a transfer call tool, configure its destinations, and choose a transfer mode.
</Card>
...
Use an API Request Tool to call an HTTP endpoint during a conversation and make the response available to the assistant.

<Card title="API Request Tool" icon="fa-light fa-globe" href="/tools/api-request">
  Build a working coffee-order example, configure requests, handle errors and retries, and use response data in later tools.
</Card>
